...
int minCost = INT_MAX; // Variable to store the minimum cost of the tour(setting such that it will be ovevrwrite when lower value is found)
...
void tsp(int level, int cost, int path[], int visited[]) {
    ...
    if (level == n) {
        ...
        if (graph[path[level - 1]][0] != 0) //if its non zero means valid edge
        {
